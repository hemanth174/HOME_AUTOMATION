    ...
    <form className="flex flex-col gap-4" onSubmit={failedAttempts >= 3 ? handleVerifyRecoveryOTP : handleForgotVerify}>
      ...
      {failedAttempts >= 3 ? (
        <div className="flex flex-col gap-1 mt-2 border-t border-border pt-4 animate-scale-in">
          ...
          />
          <button
            className="w-full py-3 rounded-lg text-sm font-bold bg-accent text-[var(--btn-text)] transition-all hover:bg-accent-hover active:translate-y-0 cursor-pointer shadow-gold-glow mt-4"
            type="submit"
            disabled={loading}
          >
