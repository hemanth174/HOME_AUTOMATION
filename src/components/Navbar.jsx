...
export default function Navbar() {
  ...
  const [showWifiNetDropdownMobile, setShowWifiNetDropdownMobile] =
    useState(false);
  ...
  const getBoardSignal = (board, online) => {
    ...
    const percentage = Math.min(
      100,
      Math.max(0, Math.round(((rssi + 100) / 60) * 100)),
    );
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      if (
        wifiDropdownRef.current &&
        !wifiDropdownRef.current.contains(e.target)
      ) {
        setShowWifiDropdown(false);
      }
      if (
        wifiDropdownRefMobile.current &&
      ...
      if (
        wifiNetDropdownRef.current &&
        !wifiNetDropdownRef.current.contains(e.target)
      ) {
        setShowWifiNetDropdown(false);
      }
      if (
        wifiNetDropdownRefMobile.current &&
        !wifiNetDropdownRefMobile.current.contains(e.target)
      ) {
  ...
  const getPageTitle = () => {
    const pathMap = {
      "/": "Dashboard - Overview",
      "/local": "Local Mesh - Sub-30ms Control",
    ...
    <div className="flex flex-col h-full justify-between py-6 px-4 select-none">
      ...
      <div className="flex flex-col gap-4">
        ...
        <nav className="flex flex-col gap-1.5">
          <span className="text-[9px] font-extrabold uppercase tracking-widest text-text-muted px-2 mb-1">
            MAIN MENU
          </span>
          {links.map((link) => {
      ...
      <div className="flex flex-col gap-4 border-t border-border pt-4">
        ...
        <div className="flex flex-col gap-0.5 px-2">
          <span className="text-[9px] font-extrabold uppercase tracking-wider text-text-muted block">
            Timezone
          </span>
          <span
            className="text-[10px] font-bold text-text truncate"
            title={timezone}
          >
            Auto ({timezone})
          </span>
        ...
        >
          ...
          <div className="flex flex-col min-w-0 flex-1">
            <span className="text-xs font-black text-text truncate leading-none mb-1">
              {fullName}
            </span>
            <span className="text-[10px] text-text-muted truncate leading-none">
              {email}
            </span>
          </div>
        </Link>
      ...
      <header className="fixed top-0 right-0 left-0 md:left-64 h-16 border-b border-border bg-header/90 backdrop-blur-md flex items-center justify-between px-6 z-40 max-md:hidden select-none">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {showWifiNetDropdown && (
              <div className="absolute right-0 top-full mt-2.5 w-72 rounded-2xl border border-border bg-card p-4 shadow-2xl z-50 flex flex-col gap-3 animate-scale-in">
                ...
                <div className="flex flex-col gap-2 text-xs font-semibold select-none">
                  {isClientOnline ? (
                    <div className="flex flex-col gap-1.5 text-[11px] text-text-muted">
                      ...
                      {wifiIsWeak && (
                        <div
                          className={`flex items-start gap-2 p-2 rounded-lg border text-[10px] font-semibold mb-0.5 ${
                            wifiIsCritical
                  ...
                  {boards.length === 0 ? (
                    <div className="text-center py-4 flex flex-col items-center gap-2">
                      ...
                      <p className="text-[11px] text-text-muted font-bold leading-normal px-4">
                        No active ESP32 switchboards have been added. Visit the{" "}
                        <Link href="/boards" className="text-accent underline">
                          Boards Manager
                        </Link>{" "}
                        to connect a device.
                      </p>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-extrabold text-text truncate max-w-[150px]">
                              {b.name}
                            </span>
                            <span
                              className={`text-[9px] font-black uppercase tracking-wider ${online ? "text-accent" : "text-red-500"}`}
                            >
                          ...
                          {online ? (
                            ...
                            <div className="flex flex-col gap-1 text-[10px] leading-relaxed font-semibold text-red-500/90">
                              ...
                              <p className="text-[9px] text-text-muted leading-tight mt-0.5">
                                Please move the board closer to your router or
                                check if the device power is connected.
                              </p>
                            </div>
                          )}
                        </div>
                  ...
                  {boards.length === 0 ? (
                    <div className="text-center py-4 flex flex-col items-center gap-2">
                      ...
                      <p className="text-[10px] text-text-muted font-semibold leading-normal px-2">
                        No active switchboards. Add a device on the Boards
                        screen.
                      </p>
                    </div>
                  ) : (
                    boards.map((b) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-extrabold text-text truncate max-w-[130px]">
                              {b.name}
                            </span>
                            <span
                              className={`text-[8px] font-black uppercase tracking-wider ${online ? "text-accent" : "text-red-500"}`}
                            >
                              {online ? "Online" : "Offline"}
                            </span>
                          </div>
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-[9px] font-extrabold uppercase tracking-widest text-text-muted px-2">
              More Options
            </span>
            {sheetLinks.map((link) => {
              const active = pathname === link.href;
