...
export default function LocalControlPage() {
  ...
  const fetchLocalState = useCallback(
  ...
  const handleToggle = async (index, targetState) => {
    ...
    setDevices((prev) =>
      prev.map((d) =>
        d.relay_index === index ? { ...d, is_on: targetState } : d,
      ),
    );
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDevices((prev) =>
          prev.map((d) =>
            d.relay_index === index ? { ...d, is_on: data.state } : d,
          ),
        );
        showToast(
          `Switch ${index + 1} turned ${targetState ? "ON" : "OFF"} (< 30ms)`,
        );
      } else {
      ...
      showToast(
        `⚠️ Failed to switch Device ${index + 1}. Check ESP32 connection.`,
      );
      fetchLocalState(true);
    } finally {
  ...
  return (
    <div className="w-full min-h-screen bg-background text-text pt-16 sm:pt-20 pb-12 px-3.5 sm:px-6 lg:px-8 select-none">
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        ...
        {/* Connection Notice / Offline Mobile Help Banner */}
        {!isConnected && (
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3.5 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] sm:text-xs text-amber-300/80 leading-relaxed">
                  ...
                  <br />
                  3️⃣ Tap button on right to open direct local AP panel!
                </p>
              </div>
            </div>
            <a
            ...
            >
              <span>Open 192.168.4.1 Direct AP</span>
          ...
          {activeScenario === 1 && (
            <div className="text-xs text-text-muted space-y-1.5 leading-relaxed animate-scale-in">
              ...
              <ul className="list-disc pl-4 space-y-1 text-[11px] sm:text-xs">
                ...
                <li>
                  This Local Control page dispatches HTTP POST requests to{" "}
                  <code className="text-text font-mono">192.168.4.1</code> with
          ...
          {activeScenario === 2 && (
            <div className="text-xs text-text-muted space-y-1.5 leading-relaxed animate-scale-in">
              ...
              <ul className="list-disc pl-4 space-y-1 text-[11px] sm:text-xs">
                ...
                <li>
                  When home Wi-Fi connects, the ESP32 flushes its queue to
          ...
          {activeScenario === 3 && (
            <div className="text-xs text-text-muted space-y-1.5 leading-relaxed animate-scale-in">
              ...
              <ul className="list-disc pl-4 space-y-1 text-[11px] sm:text-xs">
                ...
                <li>
                  The ESP32 detects the hardware state change, toggles the
                  relay, and updates the{" "}
                  <strong className="text-emerald-400">AC ON / AC OFF</strong>{" "}
                  pill in real-time.
                </li>
              </ul>
            </div>
